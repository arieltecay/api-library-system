...
async function seedClients() {
    ...
    await ClientModel.collection.insertMany(rawClients.map(c => ({
        fullName: c.fullName,
        dni: c.dni,
        phone: c.phone,
        isDefault: c.dni === '0',
...
async function seedCashShifts(users) {
...
async function seedSales(products, clients, users, cashShifts) {
    const sellers = users.filter(u => u.role === 'seller');
    const defaultClient = clients.find((c) => c.dni === '0') ?? clients[0];
    const realClients = clients.filter((c) => c.dni !== '0');
    ...
    const salesToInsert = [];
    const creditSales = [];
    const clientBalanceMap = new Map();
    realClients.forEach((c) => clientBalanceMap.set(c._id.toString(), 0));
...
// seedCreditMovements
// ---------------------------------------------------------------------------
async function seedCreditMovements(creditSales, clients, users) {
    const admin = users.find(u => u.role === 'admin');
    if (!admin)
        return 0;
    const clientsById = new Map(clients.map((c) => [c._id.toString(), c]));
    const clientGroupedSales = new Map();
